...
class CLI:
    ...
    def __init__(self, stdin=None, stdout=None, stderr=None):
        ...
        self.parser = argparse.ArgumentParser(
            description="Run or manage HOTP/TOTP calculations", prog="authenticator"
        )
        ...
        sp_del = subparsers.add_parser(
            ...
            epilog=epilog,
            description=textwrap.fill(
                "Delete one or more HOTP/TOTP configurations from " + "the data file.",
                width=epilog_width,
            ),
        )
        sp_del.add_argument(
        ...
        sp_del.add_argument(
            ...
            action="store_true",
            default=False,
            help="clientIdPattern is a regular expression",
        )
        sp_del.add_argument(
            "-q",
            "--quiet",
        ...
        sp_gen.add_argument(
            ...
            action="store_true",
            default=False,
            help="client_id_pattern is a regular expression",
        )
        sp_gen.add_argument(
            "--refresh",
            dest="refresh",
        ...
        sp_gen.add_argument(
            "-c",
            "--counter-based",
            dest="includeCounterBasedConfigs",
            action="store_true",
            default=False,
        ...
        sp_info = subparsers.add_parser(
            "info",
            help="show information about this software and your data",
            formatter_class=argparse.RawDescriptionHelpFormatter,
            description=textwrap.fill(
        ...
        sp_list = subparsers.add_parser(
            "list",
            help="list HOTP/TOTP configurations",
            formatter_class=argparse.RawDescriptionHelpFormatter,
            epilog=epilog,
            description=textwrap.fill(
                "List one or more HOTP/TOTP configurations from the " + "data file.",
                width=epilog_width,
            ),
        )
        ...
        sp_set = subparsers.add_parser(
            "set",
            help="set HOPT configuration values",
            formatter_class=argparse.RawDescriptionHelpFormatter,
            description=textwrap.fill(
        ...
        sp_set_passphrase = setsubparsers.add_parser(
            ...
            description=textwrap.fill(
                "Change the passphrase for the data file.", width=epilog_width
            ),
        )
    ...
    def _capture_passphrase(self, is_new=False):
        ...
        pp2 = None
        first_time = True
        while pp1 is None or pp2 is None or pp1 != pp2:
            if not first_time:
                print("Passphrases do not match. Try again.", file=self.__stdout)
            first_time = False
            # Prompt for new passphrase
            #
            if self._stdin_is_tty() and sys.stdin is self.__stdin:
                pp1 = getpass.getpass(
                    "Enter {0}passphrase: ".format(infix_prompt), stream=self.__stdout
                )
            else:
                print(
            ...
            if self._stdin_is_tty() and sys.stdin is self.__stdin:
                pp2 = getpass.getpass(
                    "Confirm {0}passphrase: ".format(infix_prompt), stream=self.__stdout
                )
            else:
                print(
    ...
    def _confirm_delete(self, client_id):
        rc = self._query_prompt("Delete {0}?".format(client_id), default_value_index=1)
        if "yes" == rc:
            return True
        return False
    ...
    def _delete_client_data_from_file(self, id_pattern="*"):
        ...
        #
        cds_to_delete = list(filter(lambda x: self._match_clientid(x, id_pattern), cds))
        ...
        if not self.args.quiet:
            provisional_client_ids = list(
                filter(lambda x: self._confirm_delete(x), client_ids_to_delete)
            )
    ...
    def _generate(self, id_pattern="*", refresh=5):
        ...
        #
        cds_to_calc = list(filter(lambda x: self._match_clientid(x, id_pattern), cds))
    ...
    def _list_client_data_verbose(self, cd):
        ...
        if cd.counter_from_time():
            print("time-based; period: {0}".format(cd.period()), file=self.__stdout)
        else:
            print(
                "counter-based; last counter: {0}".format(cd.last_count()),
                file=self.__stdout,
            )
            t = iso8601.parse_date(cd.last_count_update_time())
            print(
                "count updated: {0}".format(t.strftime(self.__std_fmt)),
                file=self.__stdout,
            )
        print("password length: {0}".format(cd.password_length()), file=self.__stdout)
        if 0 < len(cd.tags()):
            print("tags: {0}".format(", ".join(cd.tags())), file=self.__stdout)
        if 0 < len(cd.note()):
            print("note: {0}".format(", ".join(cd.note())), file=self.__stdout)
    ...
    def _list_client_data(self, id_pattern="*"):
        ...
        cds = self.__cf.load(self.__data_file)
        cds_to_list = list(filter(lambda x: self._match_clientid(x, id_pattern), cds))
    ...
    def _query_passphrase(self):
        ...
        while self.__passphrase is None:
            if not first_time:
                print("Passphrase is incorrect. Try again.", file=self.__stdout)
            # Capture the passphrase
            #
            first_time = False
            if self._stdin_is_tty() and sys.stdin is self.__stdin:
                pp = getpass.getpass("Enter passphrase: ", stream=self.__stdout)
            else:
                print("Enter passphrase: ", end="", flush=True, file=self.__stdout)
    ...
    def _query_shared_secret(self):
        while self.__shared_secret is None:
            print("Enter shared secret: ", end="", flush=True, file=self.__stdout)
            ...
            try:
                ...
                print(
                    "Invalid shared secret string. {0}.".format(e), file=self.__stdout
                )
                continue
            self.__shared_secret = ss

    def _query_prompt(
        self, prompt_statement, possible_values=("yes", "no"), default_value_index=0
    ):
    ...
    def _rename_client_id(self, old_client_id, new_client_id):
        ...
        if not found_id:
            print(
                "No configuration found with client ID '{0}'".format(old_client_id),
                file=self.__stderr,
            )
            return False
        self.__cf.save(self.__data_file, cds_new)
    ...
    def _show_info(self):
        ...
        print(
            "MIT License. See https://opensource.org/licenses/MIT", file=self.__stdout
        )
        print("\nData file location: {0}".format(self.__data_file), file=self.__stdout)
        addendum = textwrap.fill(
            textwrap.dedent(
                """
            See https://github.com/jenesuispasdave/github/ for the source code
            repository, the latest version, and technical support."""
            ).strip(),
            width=78,
        )
    ...
    def _validate_args_add(self):
        ...
        if self.args.counter is not None and self.args.period is not None:
            self.parser.error("--counter and --period are mutually exclusive")
    ...
    def _validate_args_set(self):
        ...
        if "clientid" == self.args.subsubcmd:
            if "*" in self.args.oldClientId:
                self.parser.error("oldClientId must be an exact match; no wildcards")
            if "*" in self.args.newClientId:
                self.parser.error("newClientId must not be a wildcard string")
    ...
    def _validate_args_data_file(self):
        ...
        if not self._apply_alt_data_file_path(self.args.altDataFile):
            self.parser.error("The directory for the data file does not exist.")
    ...
    def create_data_file(self):
        ...
        if "add" != self.args.subcmd:
            print(
                "No data file was found; cannot complete request.", file=self.__stdout
            )
            self.__abandon_cli = True
            return
        ...
        #
        prompt = "No data file was found." + " Do you want to create your data file?"
    ...
    def parse_command_args(self, args):
        ...
        if self.__stdout_redirected and self.__stderr_redirected:
            with CLI.RedirectStdStreams(stdout=self.__stdout, stderr=self.__stderr):
    ...
    def prompt_for_secrets(self):
        ...
        #
        if self.args.subcmd in ("add", "delete", "generate", "list", "set"):
        ...
        #
        if ("set" == self.args.subcmd) and ("passphrase" == self.args.subsubcmd):
    ...
    def _execute_add(self):
        ...
        try:
            ...
            print("Add failed. That configuration already exists.", file=self.__stdout)
            return
        print("OK", file=self.__stdout)

    def _execute_delete(self):
        """Execute the delete action."""
        delete_count = self._delete_client_data_from_file(self.args.clientIdPattern)
        if 0 == delete_count:
            ...
            print(
                "Deleted {0} configurations.".format(delete_count), file=self.__stdout
            )
    ...
    def _execute_set(self):
        ...
        if "clientid" == self.args.subsubcmd:
            ...
            print(
                "'set {0}' is not implemented.".format(self.args.subsubcmd),
                file=self.__stdout,
            )
    ...
    def _execute_subcmd(self):
        ...
        if "add" == self.args.subcmd:
            ...
            print(
                "'{0}' is not implemented.".format(self.args.subcmd), file=self.__stdout
            )
    ...
    def execute(self):
        ...
        if self.args.subcmd not in ("add", "delete", "generate", "info", "list", "set"):
            print(
                "'{0}' is not implemented.".format(self.args.subcmd), file=self.__stdout
            )
            return
        ...
        if "set" == self.args.subcmd:
            if self.args.subsubcmd not in ("passphrase", "clientid"):
                print(
                    "'set {0}' is not implemented.".format(self.args.subsubcmd),
                    file=self.__stdout,
                )
